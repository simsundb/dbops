...
public class GenerateScriptPanel extends JPanel {
    ...
    private void initTableActions() {
        ...
        detailTable.addMouseListener(new MouseAdapter() {
            ...
            public void mouseClicked(MouseEvent e) {
                if (e.getClickCount() == 2) {
                    int viewRow = detailTable.rowAtPoint(e.getPoint());
                    if (viewRow >= 0) {
                        toggleExecFlag(viewRow);
                    }
                }
            }
        });
        ...
        detailTable.addMouseListener(new MouseAdapter() {
            ...
            public void mouseClicked(MouseEvent e) {
                int viewRow = detailTable.rowAtPoint(e.getPoint());
                int viewCol = detailTable.columnAtPoint(e.getPoint());
    ...
    /**
     * 切换指定视图行的执行标志（自动转换视图索引到模型索引）
     */
    private void toggleExecFlag(int viewRow) {
        int modelRow = detailTable.convertRowIndexToModel(viewRow);
        DataCheckDetail detail = detailTableModel.getRowData(modelRow);
        String currentFlag = detail.getExecFlag();
        String newFlag = "Y".equals(currentFlag) ? "N" : "Y";
        detail.setExecFlag(newFlag);
        detailTableModel.fireTableRowsUpdated(modelRow, modelRow);
    ...
    private void editSelected() {
        int viewRow = detailTable.getSelectedRow();
        if (viewRow < 0) {
