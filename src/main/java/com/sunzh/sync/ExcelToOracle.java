...
public class ExcelToOracle {
    ...
    public static void main(String[] args) throws Exception {
        ...
        String tableName = toOracleTableName(baseName);
        ...
        try (Connection conn = DriverManager.getConnection(dbUrl, dbUser, dbPassword);
            ...
            for (int i = 0; i < colNames.size(); i++) {
                log(String.format("  %-30s VARCHAR2(%d)", colNames.get(i), maxLen[i]));
            }
    ...
    /** Oracle 表名: 字母开头, 去掉特殊字符, 限制30字符 */
    private static String toOracleTableName(String baseName) {
        StringBuilder sb = new StringBuilder();
        HanyuPinyinOutputFormat format = new HanyuPinyinOutputFormat();
        format.setToneType(HanyuPinyinToneType.WITHOUT_TONE);
    ...
    private static String buildCreateDdl(String tableName, List<String> colNames, int[] maxLen) {
        ...
        for (int i = 0; i < colNames.size(); i++) {
            sb.append("  \"").append(colNames.get(i)).append("\" VARCHAR2(").append(maxLen[i]).append(")");
